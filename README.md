...
Open PRs are linked from the repository's Pull requests tab so the red vs green contrast is visible.

## Run it locally (offline)

```bash
# needs python3 and the assay-mcp-server binary: https://github.com/Rul1an/assay/releases
ASSAY=/path/to/assay-mcp-server ./scripts/run-gate.sh
```
